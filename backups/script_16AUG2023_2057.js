...
fetch('./assets/map_data/precipitation/2018-07-17_IMERG.asc')
    .then(response => response.text())
    .then(data => {
        ...
        // Filter out NaN values and calculate max precipitation
        const validValues = values.flat().filter(value => !isNaN(value));
        const maxPrecipitation = Math.max(...validValues);
        console.log(values)
            ...
            for (let row = 0; row < nrows; row++) {
                ...
                if (precipitation !== noDataValue) {
                    L.circleMarker([lat, lon], {
                        ...
                        color: 'NONE',
                        fillColor: 'darkblue',
                        fillOpacity: getMarkerAlpha(precipitation, maxPrecipitation)
                    }).addTo(map);
                }
            }
        }
