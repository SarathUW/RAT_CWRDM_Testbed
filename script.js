...
var cartoDarkLayer = L.tileLayer(
...
var stamenTonerLayer = L.tileLayer(
...
var stamenTerrainLayer = L.tileLayer(
...
var cartoDBPositron = L.tileLayer(
  ...
  .then((data) => {
    ...
    geoJSONLayer = L.geoJSON(data, {
      style: function (feature) {
        return {
          fillColor: "cyan", // Set initial fill color to transparent green
          color: "skyblue", // Set outline color to black
...
fetch("./assets/map_data/dam_locations.csv")
  ...
  .then((data) => {
    ...
    //custom marker for dams
    var customIcon = L.icon({
      iconUrl: "./assets/images/damMarker.png",
      iconSize: [40, 40],
      iconAnchor: [20, 40],
      popupAnchor: [0, -40],
    });
    ...
    for (var i = 1; i < rows.length - 1; i++) {
      ...
      popupContent.innerHTML =
        damNameDiv.outerHTML +
        '<button class="btn btn-outline-success btn-sm" onclick="showPlots(\'' +
        name.replace(/\s/g, "") +
        "', '" +
        mrbid.trim() +
        "')\"><p3>View Data</p3></button>";
...
function showPlots(dam_name, mrbID) {
  ...
  inflowFilename =
    "./assets/RAT_results/inflow/" + dam_name.toLowerCase() + ".csv";

  aecFilename = "./assets/RAT_results/aec/" + dam_name.toLowerCase() + ".csv";

  evapFilename =
    "./assets/RAT_results/evaporation/" + dam_name.toLowerCase() + ".csv";

  SA_MNDWIFilename =
    "./assets/RAT_results/sarea_tmsos/" + dam_name.toLowerCase() + "_MNDWI.csv";
  SA_NDWIFilename =
    "./assets/RAT_results/sarea_tmsos/" + dam_name.toLowerCase() + "_NDWI.csv";
  sa_Filename =
    "./assets/RAT_results/sarea_tmsos/" + dam_name.toLowerCase() + ".csv";

  outflow_MNDWIFilename =
    "./assets/RAT_results/outflow/" + dam_name.toLowerCase() + "_MNDWI.csv";
  outflow_NDWIFilename =
    "./assets/RAT_results/outflow/" + dam_name.toLowerCase() + "_NDWI.csv";
  outflow_Filename =
    "./assets/RAT_results/outflow/" + dam_name.toLowerCase() + ".csv";

  delS_Filename =
    "./assets/RAT_results/dels/" + dam_name.toLowerCase() + ".csv";

  temp_Filename =
    "./assets/WQ/temperature/" + mrbID + "_" + dam_name + "_RES.csv";
  ...
  document
    .getElementById("temperatureRadio")
    .addEventListener("click", function () {
...
function showInflow(inflowFilename) {
  if (offcanvastriggered == false) {
    var offcanvas = new bootstrap.Offcanvas(
      document.getElementById("graphOffcanvas")
    );
    offcanvas.show();
...
// Function for surface area plotting
    ...
    .then((data) => {
      ...
      var layout = {
        ...
        yaxis: {
          title: "Elevation (ft)",
          range: [
            dam_aecShift[dam_name] - 200,
            elevation_feet[elevation_feet.length - 2] + 200,
          ],
        },
        xaxis: {
...
// Get the current date -2 - Uncomment next 3 lines once data pipeline is set up
// let currDateMinus2 = new Date();
// currDateMinus2.setDate(currDateMinus2.getDate() - 5);
// currDateMinus2 = currDateMinus2.toISOString().split("T")[0];

currDateMinus2 = "2024-07-07"; // Comment this out once data pipeline is set up

function plotPrecipitation(selectedDate = currDateMinus2) {
  var maxPrecip = 0;
  var minPrecip = 200;
  var precipMinMax = {};
...
function plotColorBar(min, max) {
  ...
  map
    ...
    .appendChild(colorBarLegendContainer);
}
...
function createDatePicker() {
  var DatePickerControl = L.Control.extend({
    onAdd: function (map) {
      ...
      container.innerHTML = `
            <div class="toggle-box bg-dark px-2 py-0 rounded">
               <input type="date" id="datePicker" value="${currDateMinus2}" class="form-control bg-dark">
            </div>
         `;
      return container;
    },
